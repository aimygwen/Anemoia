 ...
 * SPA: AboutPage.boot(scope) / AboutPage.unmount()
 */
(function () {
  "use strict";
  ...
  function boot(scope) {
    ...
    var root = scope.querySelector(".about");
    if (!root) return;

    var curtain = root.querySelector(".scroll-curtain");
    var curtainPath = root.querySelector(".scroll-curtain__shape");
    var underlay = root.querySelector(".about-underlay");
    var polaroid = underlay && underlay.querySelector("img");
    var hero = root.querySelector(".about-hero");
    var hudStack = root.querySelector("[data-about-hud-stack]");
    var hudIntro = root.querySelector(".ins-intro");
    var hudCanvas = root.querySelector("[data-about-hud-canvas]");
    var hudPixelWrap = root.querySelector("[data-about-hud-pixel]");
    ...
    function bootAboutReveals() {
      var sheet = root.querySelector("[data-about-sheet]");
      if (!sheet) return;
    ...
    if (!reduced && window.Polyglide) {
      lenis = window.__lenis || window.Polyglide.boot();
      if (lenis) {
    ...
    active = {
      root: root,
      stopRaf: stopRaf,
      hudPixelAnimId: hudPixelAnimId,
      hudPixelWaitId: hudPixelWaitId,
      lenis: lenis,
      syncScroll: syncScroll,
      onPointerMove: onPointerMove,
      onPointerLeave: onPointerLeave,
      scrollListenerOnWindow: !!(reduced || !window.Polyglide)
    };
  }

  function unmount() {
    if (!active) return;
    if (typeof active.stopRaf === "function") active.stopRaf();
    if (active.hudPixelAnimId) cancelAnimationFrame(active.hudPixelAnimId);
    if (active.hudPixelWaitId) window.clearInterval(active.hudPixelWaitId);
    if (active.lenis && active.syncScroll && typeof active.lenis.off === "function") {
      active.lenis.off("scroll", active.syncScroll);
    }
    if (active.scrollListenerOnWindow && active.syncScroll) {
      window.removeEventListener("scroll", active.syncScroll);
    }
    if (active.onPointerMove) window.removeEventListener("pointermove", active.onPointerMove);
    if (active.onPointerLeave) document.documentElement.removeEventListener("pointerleave", active.onPointerLeave);
    if (active.syncScroll) window.removeEventListener("resize", active.syncScroll);
    active = null;
  }

  window.AboutPage = { boot: boot, unmount: unmount };
